...
- **任意表单的字段名匹配推荐**：
  复制的文本里有「字段名: 值」这类行（单据、工单、导出记录、OCR 结果，值写在下一行也行），输入框标签对得上字段名就推荐对应的值，不需要事先知道字段类型，中英文标签都支持。
- **内置智能报销审核网页 Demo**：
  项目内含基于 Vercel Geist 设计风格的中英双语报销审核工作台（`demo/index.html` 中文，`demo/index-en.html` 英文），可一键在默认浏览器中打开体验。
- **纯菜单栏常驻（不占程序坞）**：
